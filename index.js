...
const { loadPlugins } = require('./plugins/bot');
...
async function startBot() {
  ...
  try {
    ...
    const { useMultiFileAuthState, getContentType, fetchLatestBaileysVersion, DisconnectReason, Browsers } = baileys;
    ...
    sock.ev.on('connection.update', async update => {
      ...
      if (connection === 'close') {
        ...
        if (code !== DisconnectReason.loggedOut) {
          ...
          console.error('❌ Logged out. Delete auth folder to relogin.');
        }
        ...
        try {
          ...
          for (let owner of ownerNumber) {
            await sock.sendMessage(owner + '@s.whatsapp.net', { text: '✅ QUEEN-HASUKI MINI is now online!' });
          }
        } catch (err) {
...
const startServer = async () => {
  try {
    ...
    // Start bot **only if running locally**
    if (!process.env.VERCEL) {
      await startBot();
    } else {
      console.log(chalk.yellow('⚠️ WhatsApp bot disabled on Vercel (serverless).'));
    }
